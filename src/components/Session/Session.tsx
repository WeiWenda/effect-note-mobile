...
import {promiseDebounce, promiseNoConcurrentRun} from '../../ts/utils/functional';
...
export default class SessionComponent extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.props.session.on('importFinished', async (_path: Path) => {
      this.setState({forceLoadCounter: 1});
    });
  ...
  fetchAndRerender = promiseDebounce(async () => {
    console.log('fetchAndRerender');
    if (this.state.forceLoadCounter > 50) {
      return;
    } else {
      this.setState({
        forceLoadCounter: this.state.forceLoadCounter + 1
      });
    }
    const session = this.props.session;
    const finishProfiling = this.getProfiler(this.profileRender)('forceLoadTree took');
    await session.document.forceLoadTree(session.viewRoot.row);
    finishProfiling();
    this.update();
  })
